...
const Six = () => {
  ...
  const { t } = useTranslation();
   ...
   const { select } = useContext(Context);
    ...
    return (
       <>
<div dir={select===true?'rtl':'ltr'} className={` grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 place-items-center gap-6 m-auto mt-10`} data-aos="fade-up">
...
<div className='flex flex-col'>
 <div className=' border-2 border-blue-700  w-max px-4 py-1 text-center text-blue-700 rounded-lg hover:bg-blue-700 hover:text-white m-auto mb-1 cursor-pointer'>{t('whyChooseUs')}</div>
  ...
  <div className=''>
   <video src={newImage} autoPlay loop muted playsInline disablePictureInPicture className='rounded-full w-[45rem]'  data-aos ="zoom-in" ></video>
</div>
